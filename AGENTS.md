...
**Node version gotcha:** the VM's *default* `node` may be v22, but this repo requires Node **24** (`.nvmrc`, `engines`). Run `nvm use 24` (or `nvm alias default 24`) before any `npm` command — otherwise installs/builds run under the wrong Node.

**Overlayfs `EXDEV` gotcha on `npm run build`:** on a fresh checkout the tracked `app/api` dir sits on the overlay *lower* layer, so `scripts/build-static.mjs`'s `fs.renameSync(app/api → .api-stash)` fails with `EXDEV: cross-device link not permitted`. Force `app/api` onto the upper layer once before building: `cp -a app/api /tmp/api && rm -rf app/api && cp -a /tmp/api app/api && rm -rf /tmp/api` (no tracked-file changes). `npm run build:api` uses copy+delete instead of rename, so it is unaffected. `npm run dev` does not stash/rename and is also unaffected.
