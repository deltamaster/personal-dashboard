...
**Dedicated QA cloud stack (Terraform):** `terraform/qa/` is a separate root that provisions an isolated test OTS instance + OSS buckets + a **media CDN** (`pd-dash-qa`, `pd-web-qa`, `pd-vault-qa`, CDN `pd-qa.huhansen.com`) with its own state — no FC (QA API runs locally), never touches prod. Apply via **Actions → Terraform QA → apply**, then `node scripts/qa-seed.mjs` clones the schema + seeds dummy rows (it refuses prod instance names). The live `pd-dash-sg` is **production data**, not a test env — don't seed/pollute it. See `terraform/README.md`.
